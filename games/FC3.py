...
class FC3(gym.Env):
    ...
    def step(self, actions): # 1 is dare, 0 is chicken out
        ...
        bool_R = np.identity(3,dtype=bool)
        R = np.zeros((3,3))
        for i in range(3):
            for j in range(3):
        ...
        rews = np.sum(R, axis=1)/2
        done = self.t >= self.horizon
        return [np.array([0.0]), np.array([0.0]), np.array([0.0])], rews, done, {}
    
    @property
    def action_space(self):
        if self.gifters == 'none':
            return [Discrete(2), Discrete(2), Discrete(2)]
        elif self.gifters == 'all':
            return [Discrete(4), Discrete(4), Discrete(4)]
        else:
